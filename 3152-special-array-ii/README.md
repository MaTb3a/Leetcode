...
<p>You are given an array of integer <code>nums</code> and a 2D integer matrix <code><bvtag class="memorize_new_word bv_leetcode_com bv_browserName_google_chrome  burning_vocabulary  _id_1733761775848">queries</bvtag></code>, where for <code>queries[i] = [from<sub>i</sub>, to<sub>i</sub>]</code> your task is to check that <span data-keyword="subarray">subarray</span> <code>nums[from<sub>i</sub>..to<sub>i</sub>]</code> is <strong>special</strong> or not.</p>
...
<ul>
	...
	<li><code>0 &lt;= queries[i][0] &lt;= <bvtag class="memorize_new_word bv_leetcode_com bv_browserName_google_chrome  burning_vocabulary  _id_1733761775848">queries</bvtag>[i][1] &lt;= nums.length - 1</code></li>
</ul>
</div>